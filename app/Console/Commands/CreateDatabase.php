...
//        dd($str_len);
        ...
        foreach ($database_names as $db_name) {
            ...
            try {
                ...
                $statement = "CREATE DATABASE IF NOT EXISTS {$db_name};";
                $res       = $db->exec($statement);
                if ($res === false)
                {
                    throw new \Exception("不正なSQL文 - {$statement}");
                }
                $this->info(sprintf("%-{$str_len}s", $db_name) . " : 正常に作成されました。");
            } catch (\Exception $e) {
                $this->error(sprintf("%-{$str_len}s", $db_name) . " : 作成されませんでした。（理由：{$e->getMessage()}）");
            }
        }
    }
