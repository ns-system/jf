        ...
        <div class="panel-footer">
            @if(isset($r->is_plan_accept) && !$r->is_plan_accept && $r->is_plan_entry)
                ...
                <span class="btn-group" data-toggle="buttons">
                    ...
                    <label class="btn btn-xs btn-default activate"   data-color="btn-success" data-target="#actual-form-{{$r->key_id}}"><input type="checkbox" name="plan[{{$r->key_id}}]" value="1">承認</label>
                    <label class="btn btn-xs btn-default inactivate" data-color="btn-danger"  data-target="#actual-form-{{$r->key_id}}"><input type="checkbox" name="plan[{{$r->key_id}}]" value="0">却下</label>
...
@if(isset($r->is_actual_entry) && $r->is_actual_entry)
    @if(isset($r->is_plan_entry) && $r->is_plan_entry)
    <div
        class="text-right"
        id="actual-form-{{$r->key_id}}"
        style="display: none;"
    >
