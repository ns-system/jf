    ...
    <div class="container-fluid">
        ...
        <div class="border-bottom">
            <h2>
                CSVファイルアップロード処理
                <small>
                   - {{date('Y年n月', strtotime($id.'01'))}} 
                   @if($term_status === 'monthly')    <label class="label label-success">月次処理</label>
                   @elseif($term_status === 'weekly') <label class="label label-success">週次処理</label>
                   @elseif($term_status === 'daily')  <label class="label label-success">日次処理</label>
                   @else
                   @endif
               </small>
           </h2>
       ...
       <form method="POST" action="{{route('admin::super::term::import_dispatch', ['term_status'=>$term_status, 'id'=>$id, 'job_id'=>$job_id])}}">
    ...
    function validateCheck(){
        ...
        if(check_count <= 0){
            ...
            if({{$term_status}} === 'daily' || {{$term_status}} === 'weekly'){
                return confirm('現在累積されている日次・週次データは全て削除されます。処理を継続してよろしいですか？');
            }else{
