...
class FuncSuisinAdminControllerConsignorSettingTest extends TestCase
{
     ...
     * @tests
     */
    public function 全オン還元CSVファイル設定を表示できる() {
     ...
     * @tests
     */
    public function 全オン還元CSVファイル設定でCSVファイルインポートできる() {
        $user      = factory(\App\User::class)->create(['is_super_user' => '1']);
        ...
        for ($i = 1; $i < count($csv_file); $i++) {
            ...
            $this->assertEquals(\App\ZenonCsv::
                            ...
                            where('is_cumulative', trim($data[11]))->
                            where('is_account_convert', trim($data[12]))->
                            where('is_process', trim($data[13]))->
                            where('is_split', trim($data[14]))->
                            where('zenon_format_id', trim($data[15]))->
                            where('account_column_name', trim($data[16]))->
                            where('subject_column_name', trim($data[17]))->
                            where('split_foreign_key_1', trim($data[18]))->
     ...
     * @tests
     */
    public function 全オン還元CSVファイル設定で内容に不備のあるCSVファイルがインポートされたときエラー() {
        $user      = factory(\App\User::class)->create(['is_super_user' => '1']);
        \App\ZenonCsv::truncate();
        $file_name = '全オン還元CSVファイル設定.csv';
     ...
     * @tests
     */
    public function 全オン還元CSVファイル設定で誤ったCSVファイルがインポートされたときエラー() {
        $user      = factory(\App\User::class)->create(['is_super_user' => '1']);
        \App\ZenonCsv::truncate();
     ...
     * @tests
     */
    public function テーブルカラム設定を表示できる() {
        $user = factory(\App\User::class)->create(['is_super_user' => '1']);
        \App\ZenonTable::truncate();
        $this->actingAs($user)
     ...
     * @tests
     */
    public function テーブルカラム設定でCSVファイルインポートできる() {
        $user      = factory(\App\User::class)->create(['is_super_user' => '1']);
        \App\ZenonTable::truncate();
        \App\ZenonCsv::truncate();
     ...
     * @tests
     */
    public function テーブルカラム設定で内容に不備のあるCSVファイルがインポートされたときエラー() {
        $user      = factory(\App\User::class)->create(['is_super_user' => '1']);
        \App\ZenonTable::truncate();
        \App\ZenonCsv::truncate();
        \App\ZenonCsv::insert($this->dummy_zenon_csv_data);
     ...
     * @tests
     */
    public function テーブルカラム設定で誤ったCSVファイルがインポートされたときエラー() {
        $user      = factory(\App\User::class)->create(['is_super_user' => '1']);
        \App\ZenonTable::truncate();
        \App\ZenonCsv::truncate();
        \App\ZenonCsv::insert($this->dummy_zenon_csv_data);
     ...
     * @tests
     */
    public function 全オンカテゴリ名を表示できる() {
        $user = factory(\App\User::class)->create(['is_super_user' => '1']);
        \App\ZenonType::truncate();
        $this->actingAs($user)
     ...
     * @tests
     */
    public function 全オンカテゴリ名でCSVファイルインポートできる() {
        $user      = factory(\App\User::class)->create(['is_super_user' => '1']);
        \App\ZenonType::truncate();
     ...
     * @tests
     */
    public function 全オンカテゴリ名で内容に不備のあるCSVファイルがインポートされたときエラー() {
        $user      = factory(\App\User::class)->create(['is_super_user' => '1']);
        \App\ZenonType::truncate();
        $file_name = '全オン還元データ種類.csv';
        $path      = storage_path() . '/tests/csvUploadFailedTestFile/' . $file_name;
     ...
     * @tests
     */
    public function 全オンカテゴリ名で誤ったCSVファイルがインポートされたときエラー() {
        $user      = factory(\App\User::class)->create(['is_super_user' => '1']);
        \App\ZenonType::truncate();
        $file_name = 'どれとも異なる設定ファイル.csv';
        $path      = storage_path() . '/tests/csvUploadFailedTestFile/' . $file_name;
